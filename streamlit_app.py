...
# --- Добавляем стрелочки и дату над графиком ---

col_prev, col_label, col_next = st.columns([1, 4, 1])
with col_prev:
    if st.button("⬅️", key="prev_date_graphic"):
        change_date(-1)
with col_label:
    st.markdown(
        f"<div style='text-align:center; font-size:20px; font-weight:600;'>📅 {selected_date}</div>", 
        unsafe_allow_html=True
    )
with col_next:
    if st.button("➡️", key="next_date_graphic"):
        change_date(1)
